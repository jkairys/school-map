...
from listings_scraper.suburb_autocomplete import autocomplete
from listings_scraper.suburb_resolver.exceptions import AutocompleteUnavailableError
...
async def test_autocomplete_passthrough_returns_all_suburb_candidates(httpx_mock):
    ...
    assert all(m.slug for m in result)
    ...
    *,
    ...
    async with factory() as s:
        async with s.begin():
            ...
            prop = Property(
                external_property_id=f"s06-prop-{tag}",
            ...
            listing = Listing(
                ...
                suburb_id=suburb_id,
                category=category,
                external_listing_id=f"oth-s06-{prop.id}",
            )
            s.add(listing)
            await s.flush()
...
async def test_properties_list_rollup_null_when_no_snapshots(api_client, session_factory):
    ...
    async with session_factory() as s:
        async with s.begin():
            prop = Property(
                external_property_id="no-snap-prop",
...
async def test_property_detail_multiple_campaigns(api_client, session_factory):
    ...
    async with session_factory() as s:
        async with s.begin():
            ...
            prop = Property(
                external_property_id="multi-campaign-prop",
...
async def test_property_detail_listing_with_no_snapshot(api_client, session_factory):
    ...
    async with session_factory() as s:
        async with s.begin():
            prop = Property(
                external_property_id="no-snap-listing-prop",
...
async def test_list_properties_includes_bedrooms_and_land(api_client, session_factory):
    ...
    async with session_factory() as s:
        async with s.begin():
            prop = Property(
                external_property_id="beds-land-prop",
            ...
            listing = Listing(
                ...
                suburb_id=sub_id,
                category="forsale",
                external_listing_id="oth-beds-land",
            )
            s.add(listing)
            await s.flush()
    ...
    async with session_factory() as s:
        async with s.begin():
            null_prop = Property(
                external_property_id="no-beds-land-prop",
...
async def test_property_detail_includes_bedrooms_and_land(api_client, session_factory):
    ...
    async with session_factory() as s:
        async with s.begin():
            prop = Property(
                external_property_id="detail-beds-land-prop",
            ...
            listing_with = Listing(
                ...
                suburb_id=sub_id,
                category="forsale",
                external_listing_id="oth-dbl-with",
            )
            s.add(listing_with)
            await s.flush()
            snap = ListingSnapshot(
            ...
            listing_no_snap = Listing(
                ...
                suburb_id=sub_id,
                category="forrent",
                external_listing_id="oth-dbl-nosnap",
            )
