...
from listings_scraper.scripts.backfill_sale_dates import backfill_sale_dates
    ...
    async with session_factory() as session:
        async with session.begin():
            row = Suburb(
                ...
                postcode=postcode,
                state=state,
                slug=f"{name.lower()}-{state.lower()}-{postcode}",
            )
            session.add(row)
            await session.flush()
            return row.id
...
async def _seed_property(
    ...
    *,
    ...
    async with session_factory() as session:
        async with session.begin():
            row = Property(
                external_property_id=f"OTH-{address[:8]}",
