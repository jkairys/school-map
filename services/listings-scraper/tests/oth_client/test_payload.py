...
from listings_scraper.oth_client.payload import build_search_payload
...
def test_state_is_uppercased() -> None:
    suburb = ResolvedSuburb(
        id=2, name="Bondi", state="nsw", postcode="2026",
        resolved_at=datetime(2024, 1, 1, tzinfo=timezone.utc),
    )
...
def test_suburb_name_is_lowercased() -> None:
    suburb = ResolvedSuburb(
        id=3, name="Wynnum West", state="QLD", postcode="4178",
        resolved_at=datetime(2024, 1, 1, tzinfo=timezone.utc),
    )
    payload = build_search_payload(suburb, Category.FORSALE, ListingFilters(), 0)
