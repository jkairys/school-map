...
- per-fixture parse smoke (every record decodes into a typed `VendorListing`),
- per-category invariants (status, price_kind, agent locations, etc),
...
FIXTURES = [
    pytest.param(
        "forsale_paddington_p0.json", Category.FORSALE, "current",
        id="forsale-paddington",
    ),
    pytest.param(
        "forrent_paddington_p0.json", Category.FORRENT, "current",
        id="forrent-paddington",
    ),
    pytest.param(
        "recentlysold_paddington_p0.json", Category.RECENTLYSOLD, "sold",
        id="recentlysold-paddington",
    ),
]


@pytest.mark.parametrize("filename,category,expected_default_status", FIXTURES)
def test_parse_fixture_smoke(
    ...
    for listing in result.listings:
        assert listing.external_property_id, "every record must have an external_property_id"
...
def test_parse_forsale_first_record(oth_fixtures_dir: Path) -> None:
    ...
    assert first.external_property_id == "3183143"
    ...
    # "For Sale" displayPrice has no parseable dollar figure → UNKNOWN price_kind.
    assert first.price_kind is PriceKind.UNKNOWN
...
def test_parse_forrent_first_record_has_weekly_rent_int(oth_fixtures_dir: Path) -> None:
    ...
    assert first.price_kind is PriceKind.RENT_WEEKLY
    assert first.status == "current"
...
def test_parse_recentlysold_first_record(oth_fixtures_dir: Path) -> None:
    ...
    assert first.external_property_id == "3177937"
    assert first.price == 1_950_000
    assert first.price_kind is PriceKind.PRICE
...
def test_parse_forsale_multi_agent_takes_first(oth_fixtures_dir: Path) -> None:
    ...
    page = parse_search_response(body, category=Category.FORSALE, page=0)
    by_id = {l.external_property_id: l for l in page.listings}
