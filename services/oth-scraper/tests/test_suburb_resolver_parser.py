...
from oth_scraper.suburb_resolver.exceptions import ParseError
from oth_scraper.suburb_resolver.resolver import (
    _build_slug,
    _filter_matches,
    _parse_response,
)
...
def test_parse_filters_street_and_address_rows():
    ...
    assert m.oth_slug == "little-mountain-4551"
...
def test_parse_returns_all_suburb_rows_including_richmond_hill():
    ...
    assert all(m.postcode and m.state and m.oth_slug for m in matches)
