...
from oth_scraper.db.models import Suburb
from oth_scraper.listing_reconciler import (
    reconcile_batch,
...
from oth_scraper.queue import ErrorClass, Job, JobQueue
...
async def run_job(
    ...
    *,
    ...
    page_number = 0
    while True:
        http = await scrape_session.http()
        page = await oth_client.search(
            suburb, category, filters, page_number, http
        )
        if page.listings:
    ...
    *,
    ...
    try:
        ...
        wait_shutdown = asyncio.create_task(
            shutdown_event.wait(), name="oth-worker-shutdown-wait"
        )
