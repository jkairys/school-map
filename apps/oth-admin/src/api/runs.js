 ...
 * @param {number} [params.offset=0]
 * @returns {Promise<Array>} list of JobRead objects
 */
export async function getRunJobs(runId, { status, limit = 200, offset = 0 } = {}) {
  const qs = new URLSearchParams()
  if (status != null) qs.set('status', status)
  qs.set('limit', limit)
  qs.set('offset', offset)

  const res = await fetch(`/scrape-runs/${runId}/jobs?${qs}`)
  if (!res.ok) {
    throw new Error(`getRunJobs(${runId}) failed: ${res.status} ${res.statusText}`)
  }
  return res.json()
}

/**
 * Re-enqueue failed/deadletter jobs from a run as a new run.
 * @param {number|string} runId
 * @returns {Promise<Object>} RetryFailedResult { run_id, list_id, job_ids, count }
 */
export async function retryFailed(runId) {
  const res = await fetch(`/scrape-runs/${runId}/retry-failed`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  })
  if (!res.ok) {
    const detail = await res.text()
    throw new Error(`retryFailed(${runId}) failed: ${res.status} ${detail}`)
  }
  return res.json()
}
